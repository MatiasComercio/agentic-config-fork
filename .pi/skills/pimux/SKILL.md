...
If the user explicitly invokes `pimux` or a `pimux-*` wrapper, treat that as a commitment to the pimux runtime.

- adopt the control-plane role immediately
- do only the minimum parent-side setup needed to read the protocol, prepare file-path handoff, and launch the right child or hierarchy
- route the substantive task through pimux child sessions rather than doing the real work directly in the parent
- for analysis, comparison, planning, or review requests, the parent may frame the task and create artifacts, but the actual analysis must happen inside spawned child session(s)
