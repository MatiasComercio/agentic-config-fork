...
- Create a success or failure signal with `{{MUX_ROOT}}/tools/signal.py` before returning.
...
- [ ] signal created via `{{MUX_ROOT}}/tools/signal.py`
- [ ] report includes `## Executive Summary` + `### Next Steps`
- [ ] no nested `subagent` calls
- [ ] no `tmux_agent` / `report_parent` calls
