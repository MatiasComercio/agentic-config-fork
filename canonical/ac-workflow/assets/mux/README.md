...
The generator resolves `{{MUX_ROOT}}` per harness so wrappers can reference one logical foundation root while the copied assets land in the correct package or plugin path for the current render.
