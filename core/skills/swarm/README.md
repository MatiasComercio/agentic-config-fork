...
Parallel research-to-deliverable orchestration via multi-agent swarm.
...
- [Core Patterns](#core-patterns)
  ...
  - [Monitor Agent Pattern](#monitor-agent-pattern)
...
Swarm is a multi-agent orchestration skill that decomposes complex research-to-deliverable tasks into parallel streams of specialized agents. The orchestrator NEVER executes leaf tasks itself; it ONLY decomposes, delegates, tracks, verifies, and reports.

**Key Characteristics**:
- Seven specialized agents with strict role separation
- File-based signal protocol for zero-context verification
- Mandatory async pattern for non-blocking execution
- Monitor agent acts as context firewall to prevent pollution
...
Use swarm for tasks that require:
...
Seven specialized agents operate in a clear hierarchy:

| Agent | Model Tier | Subagent Type | Primary Role |
|-------|------------|---------------|--------------|
| Monitor | haiku (low) | general-purpose | Track completion, context firewall |
...
**Purpose**: Zero-context verification of agent completion without reading outputs.
...
**Why This Exists**:
- Orchestrator verifies completion by counting .done files (~50 bytes each)
- Avoids reading agent outputs (which may be 5KB+ and polluted)
...
- User hears: "1 of 5 workers complete", "3 of 5 workers complete", "All 5 workers complete"
- Orchestrator stays clean; user receives progress feedback
...
Example: `20260130-0854-swarm-readme`
...
**Creation**:
```bash
eval "$(uv run tools/session.py 'topic-slug')"
# SESSION_DIR is now set to tmp/swarm/{timestamp}-topic-slug
```
...
**Usage**:
```bash
uv run tools/session.py "topic-slug"
# Output: SESSION_DIR=tmp/swarm/20260130-0854-topic-slug
```
...
# Failure signal
uv run tools/signal.py "$SESSION_DIR/.signals/001-name.fail" \
    --path "$OUTPUT_PATH" \
    --status fail \
    --error "API timeout"
```
...
```bash
total=$(uv run tools/verify.py "$SESSION_DIR" --action total-size)
...
```bash
# Default 1KB cap
uv run tools/extract-summary.py research/001-topic.md

# Custom cap
uv run tools/extract-summary.py research/001-topic.md --max-bytes 2048
```
...
Full parallel swarm with all phases:
