...
- Parent agent reads your OUTPUT FILE, not return value
...
6. CREATE SIGNAL
   uv run tools/signal.py "{signal_path}" --path "{report_path}" --status success
...
2. Create signal via: `uv run tools/signal.py {SIGNAL} --path {REPORT} --status success`
...
```
Read agents/spy.md for full protocol.
