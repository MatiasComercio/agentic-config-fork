...
def test_task_lifecycle_with_signals(result: TestResult):
    """Test full task lifecycle: create → working → completed via signals."""
    test_name = "test_task_lifecycle_with_signals"
    try:
...
def test_task_failure_via_signal(result: TestResult):
    """Test task failure handling via fail signal."""
    test_name = "test_task_failure_via_signal"
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            storage_dir = Path(tmpdir) / "storage"
            signals_dir = Path(tmpdir) / "signals"
...
def test_concurrent_task_submissions(result: TestResult):
    """Test multiple concurrent task submissions in same session."""
    test_name = "test_concurrent_task_submissions"
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            storage_dir = Path(tmpdir) / "storage"
...
if __name__ == "__main__":
    result = TestResult()
    test_task_lifecycle_with_signals(result)
    test_task_failure_via_signal(result)
    test_concurrent_task_submissions(result)
