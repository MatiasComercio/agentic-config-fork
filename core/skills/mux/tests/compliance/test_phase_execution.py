...
def test_phase_order_respected(inspector):
    ...
    for phase, time in phase_times.items():
        ...
        for later_phase, later_time in phase_times.items():
            later_idx = PHASE_ORDER.index(later_phase) if later_phase in PHASE_ORDER else -1
...
def test_monitor_same_phase_as_workers(inspector):
    """Verify monitors launch with workers of the same phase, not across phases."""
    # Simulate research phase with monitor
    inspector.record(
        "Task",
        {
            "run_in_background": True,
    ...
    inspector.record(
        ...
        {
            "run_in_background": True,
            "prompt": "Monitor research workers",
            "agent_type": "monitor",
            "model": "haiku",
        },
    )
    ...
    if workers and monitors:
        # Workers and monitors should be in same time batch
        worker_times = [w.timestamp for w in workers]
        monitor_times = [m.timestamp for m in monitors]

        # At least one monitor should be within 1s of workers
        for mt in monitor_times:
            close_to_worker = any(abs(mt - wt) < 1.0 for wt in worker_times)
            if not close_to_worker:
                pytest.fail("Monitor launched far from workers - should be same message")
...
def test_no_premature_consolidation(inspector):
    ...
    research_calls = [c for c in task_calls if classify_phase(c.parameters) == "research"]
    consolidation_calls = [c for c in task_calls if classify_phase(c.parameters) == "consolidation"]
