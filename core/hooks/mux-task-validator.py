...
# dependencies = []
# ///
"""MUX Compliance: Enforce run_in_background=True for Task() calls.
...
def main() -> None:
    try:
        ...
        # If Task without run_in_background, deny (exit 0, SDK reads JSON deny)
        if tool_name == "Task":
            if not tool_input.get("run_in_background", False):
                print(json.dumps({
                    "hookSpecificOutput": {
                        ...
                        "permissionDecisionReason": "MUX VIOLATION: Task() MUST have run_in_background=True"
                    }
                }))
                sys.exit(0)
