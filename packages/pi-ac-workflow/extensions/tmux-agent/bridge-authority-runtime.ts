...
export function evaluateBridgeAuthority(binding: BridgeAuthorityBinding, current: BridgeRuntimeSessionIdentity): BridgeAuthorityEvaluation {
	...
	if (binding.authoritativeLeafId) {
		if (current.leafId !== binding.authoritativeLeafId) {
			return {
				isAuthoritative: false,
	...
	} else if (binding.authoritativeSessionKey && current.sessionKey !== binding.authoritativeSessionKey) {
		return {
			isAuthoritative: false,
	...
	if (!binding.authoritativeLeafId && binding.authoritativeProcessId !== undefined && current.processId !== binding.authoritativeProcessId) {
		return {
			isAuthoritative: false,
