...
- Current tmux-agent hierarchy supervision now defaults `list` and `tree` to the current session scope, hides exited historical noise unless explicitly requested, returns structured tree metadata, blocks orchestrator `closeout` while direct tmux children remain unsettled, ships archive-backed `prune` support with dry-run plus confirmation, and auto-prunes missing/terminated entries aged at least `1d`.
