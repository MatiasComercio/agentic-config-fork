...
description: "pi-adapted roadmap coordinator. Uses roadmap-level progress mirrors, phase-owned stage artifacts, and one-layer subagent waves on top of the shared mux foundation."
...
```bash
uv run ../../assets/mux/tools/session.py --strict-runtime --session-key <key> "mux-roadmap-<topic>"
```
...
Every worker must follow `../../assets/mux/protocol/subagent.md`:
- write a report
- write a signal
