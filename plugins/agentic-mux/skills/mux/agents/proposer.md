...
5. CREATE SIGNAL
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/signal.py "{signal_path}" \
...
7. Create signal: uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/signal.py "{signal_path}" --path "{session_dir}/next-action.txt" --status success
8. Return EXACTLY: "0"
