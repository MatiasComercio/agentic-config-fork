...
# Skills: all directories in plugin skills directory
...
if [[ "$DRY_RUN" != true ]]; then
  if [[ "$COPY_MODE" == true ]]; then
    ...
    ln -sf "${CLAUDE_PLUGIN_ROOT}/agents" "$TARGET_PATH/agents"
  fi
  mkdir -p "$TARGET_PATH/.agent/workflows"
  if [[ "$COPY_MODE" == true ]]; then
    cp "${CLAUDE_PLUGIN_ROOT}/agents/spec-command.md" "$TARGET_PATH/.agent/workflows/spec.md"
  else
    ln -sf "${CLAUDE_PLUGIN_ROOT}/agents/spec-command.md" "$TARGET_PATH/.agent/workflows/spec.md"
  fi
fi
...
if [[ "$TOOLS" == "all" || "$TOOLS" == *"claude"* ]]; then
  ...
  if [[ "$DRY_RUN" != true ]]; then
    ...
    if [[ "$COPY_MODE" == true ]]; then
      cp "${CLAUDE_PLUGIN_ROOT}/commands/spec.md" "$TARGET_PATH/.claude/commands/spec.md"
    else
      ln -sf "${CLAUDE_PLUGIN_ROOT}/commands/spec.md" "$TARGET_PATH/.claude/commands/spec.md"
    fi
  fi
fi
...
if [[ "$TOOLS" == "all" || "$TOOLS" == *"gemini"* ]]; then
  ...
  if [[ "$DRY_RUN" != true ]]; then
    ...
    if [[ "$COPY_MODE" == true ]]; then
      cp "${CLAUDE_PLUGIN_ROOT}/commands/gemini/spec.toml" "$TARGET_PATH/.gemini/commands/spec.toml"
      cp -r "${CLAUDE_PLUGIN_ROOT}/commands/gemini/spec" "$TARGET_PATH/.gemini/commands/spec"
    else
      ln -sf "${CLAUDE_PLUGIN_ROOT}/commands/gemini/spec.toml" "$TARGET_PATH/.gemini/commands/spec.toml"
      ln -sf "${CLAUDE_PLUGIN_ROOT}/commands/gemini/spec" "$TARGET_PATH/.gemini/commands/spec"
    fi
  fi
fi
