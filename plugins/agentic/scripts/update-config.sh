...
LATEST_VERSION=$(cat "${CLAUDE_PLUGIN_ROOT}/VERSION")
...
is_self_hosted() {
  ...
  if [[ -f "$target/VERSION" && (-d "$target/core/commands/claude" || -d "$target/commands") && (-d "$target/core/agents" || -d "$target/agents") ]]; then
...
cleanup_invalid_nested_symlinks() {
  ...
  for skill_dir in "${CLAUDE_PLUGIN_ROOT}/skills/"*/ "$target/core/skills/"*/; do
...
sync_self_hosted_commands() {
  ...
  for cmd in "${all_cmds[@]}"; do
    local src="${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md"
    ...
    if [[ ! -L "$dest" ]]; then
      ...
      if [[ "$INSTALL_MODE" == "copy" ]]; then
        ...
        if [[ "$is_self" == true ]]; then
          ...
          (cd "$target/.claude/commands" && if [[ -f "../../core/commands/claude/$cmd.md" ]]; then ln -sf "../../core/commands/claude/$cmd.md" "$cmd.md"; elif [[ -f "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" ]]; then ln -sf "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" "$cmd.md"; fi)
        else
...
sync_self_hosted_hooks() {
  ...
  for hook_file in "${CLAUDE_PLUGIN_ROOT}/scripts/hooks/pretooluse/"*.py; do
...
# Skills: all directories in plugin skills or core/skills/
...
if [[ -L "$TARGET_PATH/.codex/prompts/spec.md" ]]; then
  ...
  if [[ "$CURRENT_TARGET" == *"spec-command.md" ]]; then
    ...
    if [[ "$INSTALL_MODE" == "copy" ]]; then
      ...
      cp "${CLAUDE_PLUGIN_ROOT}/commands/codex/spec.md" "$TARGET_PATH/.codex/prompts/spec.md"
    else
      ln -sf "${CLAUDE_PLUGIN_ROOT}/commands/codex/spec.md" "$TARGET_PATH/.codex/prompts/spec.md"
    fi
...
if [[ "$CURRENT_VERSION" == "$LATEST_VERSION" ]]; then
  if [[ "$NIGHTLY" == true ]]; then
    ...
    for cmd in "${AVAILABLE_CMDS[@]}"; do
      if [[ -f "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" ]] && [[ ! -e "$TARGET_PATH/.claude/commands/$cmd.md" ]]; then
    ...
    if [[ "$NEED_MAINTENANCE" == false ]]; then
      for skill in "${AVAILABLE_SKILLS[@]}"; do
        if [[ -d "${CLAUDE_PLUGIN_ROOT}/skills/$skill" ]] && [[ ! -e "$TARGET_PATH/.claude/skills/$skill" ]]; then
          NEED_MAINTENANCE=true
          break
        fi
      done
...
if [[ "$INSTALL_MODE" == "copy" && "$CURRENT_VERSION" != "$LATEST_VERSION" ]]; then
  ...
  if [[ -d "$TARGET_PATH/agents" && ! -L "$TARGET_PATH/agents" ]]; then
    ...
    if [[ -d "$COPY_BACKUP_DIR/agents" && "$BACKUP_COUNT" -ge "$ORIGINAL_COUNT" ]]; then
      ...
      cp -r "${CLAUDE_PLUGIN_ROOT}/agents" "$TARGET_PATH/agents"
  ...
  # Copy all commands
  for cmd_file in "${CLAUDE_PLUGIN_ROOT}/commands/"*.md; do
...
CMDS_INSTALLED=0
for cmd in "${AVAILABLE_CMDS[@]}"; do
  if [[ -f "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" ]]; then
    # Check if command not yet installed (neither symlink nor file)
    if [[ ! -e "$TARGET_PATH/.claude/commands/$cmd.md" ]]; then
      if [[ "$INSTALL_MODE" == "copy" ]]; then
        cp "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" "$TARGET_PATH/.claude/commands/$cmd.md"
      else
        if [[ "$IS_SELF_HOSTED" == true ]]; then
          # Self-hosted: use relative path per PROJECT_AGENTS.md
          (cd "$TARGET_PATH/.claude/commands" && if [[ -f "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" ]]; then ln -sf "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" "$cmd.md"; elif [[ -f "../../core/commands/claude/$cmd.md" ]]; then ln -sf "../../core/commands/claude/$cmd.md" "$cmd.md"; fi)
        else
          # Cross-project: use absolute path
          ln -sf "${CLAUDE_PLUGIN_ROOT}/commands/$cmd.md" "$TARGET_PATH/.claude/commands/$cmd.md"
        fi
      fi
      echo "  ✓ $cmd.md"
      ((CMDS_INSTALLED++)) || true
...
SKILLS_BACKUP_DIR=""
for skill in "${AVAILABLE_SKILLS[@]}"; do
  if [[ -d "${CLAUDE_PLUGIN_ROOT}/skills/$skill" ]]; then
    # Check if skill not yet installed (neither symlink nor directory)
    if [[ ! -e "$TARGET_PATH/.claude/skills/$skill" ]]; then
      if [[ "$INSTALL_MODE" == "copy" ]]; then
        cp -r "${CLAUDE_PLUGIN_ROOT}/skills/$skill" "$TARGET_PATH/.claude/skills/$skill"
      else
        if [[ "$IS_SELF_HOSTED" == true ]]; then
          # Self-hosted: use relative path per PROJECT_AGENTS.md
          (cd "$TARGET_PATH/.claude/skills" && if [[ -d "${CLAUDE_PLUGIN_ROOT}/skills/$skill" ]]; then ln -sf "${CLAUDE_PLUGIN_ROOT}/skills/$skill" "$skill"; elif [[ -d "../../core/skills/$skill" ]]; then ln -sf "../../core/skills/$skill" "$skill"; fi)
        else
          # Cross-project: use absolute path
          ln -sf "${CLAUDE_PLUGIN_ROOT}/skills/$skill" "$TARGET_PATH/.claude/skills/$skill"
        fi
      fi
      echo "  ✓ $skill"
