...
**If user selects "Yes":** Ask for repository URL, then create config after migration.
...
Run migration:
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/migrate-existing.sh" \
  [--dry-run] \
  [--force] \
  <target_path>
```
...
Report final status including feature configuration results from section 2:
