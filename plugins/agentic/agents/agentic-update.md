...
- Discover plugin version:
  - Read from `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json`
- Compare with `${CLAUDE_PLUGIN_ROOT}/VERSION`
...
if [[ -f "VERSION" && -d "${CLAUDE_PLUGIN_ROOT}/commands/claude" && -d "core/agents" ]]; then
  IS_SELF_HOSTED=true
fi
```

**For self-hosted repos, perform comprehensive symlink audit:**
1. List ALL `.md` files in `${CLAUDE_PLUGIN_ROOT}/commands/claude/`:
   ```bash
   ls ${CLAUDE_PLUGIN_ROOT}/commands/claude/*.md | xargs -n1 basename
   ```
...
3. **Compare and report missing symlinks:**
   - Any command in `${CLAUDE_PLUGIN_ROOT}/commands/claude/` MUST have a corresponding symlink
   - Report: "ERROR: Missing symlink: {command}.md"
4. **Offer to fix:**
   ```bash
   ln -sf ../../${CLAUDE_PLUGIN_ROOT}/commands/claude/{cmd}.md .claude/commands/{cmd}.md
   ```

**Why this matters:**
- Self-hosted repos are the SOURCE of truth
- New commands added to `${CLAUDE_PLUGIN_ROOT}/commands/claude/` won't work locally without symlinks
...
"${CLAUDE_PLUGIN_ROOT}/scripts/update-config.sh" <target_path>
```
...
# Pure bash - no external commands
"${CLAUDE_PLUGIN_ROOT}/scripts/update-config.sh" --force <target_path>
```

**For Browser Tool Installation (playwright-cli):**
```bash
# Pure bash - no external commands
"${CLAUDE_PLUGIN_ROOT}/scripts/update-config.sh" --browser-tool cli <target_path>
```

**For MCP Installation (legacy, add MCP to existing project):**
```bash
# Pure bash - no external commands
"${CLAUDE_PLUGIN_ROOT}/scripts/update-config.sh" --mcp playwright <target_path>
```
...
persist_agentic_path "${CLAUDE_PLUGIN_ROOT}"
```
...
```bash
# Pure bash - no external commands
"${CLAUDE_PLUGIN_ROOT}/scripts/update-config.sh" --nightly <target_path>
```
