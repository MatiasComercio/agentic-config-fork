...
## Your Role
Help users setup agentic-config in new or existing projects using the plugin-based
configuration system. The plugin root is available via `${CLAUDE_PLUGIN_ROOT}`.
...
### 4. Execute Setup
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/setup-config.sh" \
  [--type <type>] \
  [--copy] \
  [--tools <tools>] \
